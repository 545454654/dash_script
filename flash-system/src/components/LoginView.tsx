...
export default function LoginView({ onSuccess }: LoginViewProps) {
  ...
  return (
    <div className="w-full max-w-[430px] px-4 flex flex-col justify-center items-center min-h-[90vh] z-10 relative">
      ...
      {/* Dynamic Steps on Top */}
      <div className="w-full flex justify-center items-center mb-4 px-2 select-none" dir="rtl">
        <button
          onClick={() => setShowSteps(true)}
          className="flex items-center gap-1.5 text-xs font-bold text-yellow-400/90 hover:text-yellow-400 bg-[#001018]/60 px-5 py-2.5 rounded-xl border border-yellow-400/30 backdrop-blur cursor-pointer transition-all hover:scale-105"
        >
          <Send className="w-3.5 h-3.5" />
          <span>خطوات الحصول على الكود 🔑</span>
        </button>
      </div>
      ...
      >
        <h1 className="text-3xl font-black tracking-[4px] text-[#00f7ff] mb-6 drop-shadow-[0_0_15px_rgba(0,247,255,0.6)] font-mono uppercase">
          dash_script LOGIN
        </h1>
        ...
        <div className="flex flex-col gap-2.5 mt-4">
          <button
            onClick={() => setShowSteps(true)}
            className="w-full py-3 rounded-[30px] bg-yellow-500/10 border-2 border-yellow-400/40 text-yellow-400 font-bold text-sm cursor-pointer hover:bg-yellow-400/20 hover:border-yellow-400 transition-all active:scale-[0.98] select-none"
          >
            🔑 خذ الكود عن طريق الخطوات
          </button>
        </div>
